...
def analisis_resenas():
    ...
    # Extraer los datos (debes definir la función `extraer_datos_y_unir`)
    predicciones_df = extraer_datos_y_unir_2()

    # Diccionario con el mapeo de ciudades e imágenes
    ciudades_imagenes = {
        'San Francisco': '/Users/silvanaruizmedina/Desktop/Bootcamp/airbnb/sanfrancisco.jpg',
        'Miami': '/Users/silvanaruizmedina/Desktop/Bootcamp/airbnb/miami.jpg',
        'Chicago': '/Users/silvanaruizmedina/Desktop/Bootcamp/airbnb/Chicago.jpg',
        'Charlotte': '/Users/silvanaruizmedina/Desktop/Bootcamp/airbnb/Charlotte.jpg'
    }

    # Iterar sobre cada ciudad y mostrar la imagen y los datos individuales de cada Airbnb
    for ciudad, imagen_path in ciudades_imagenes.items():
        ...
        with col1:
            st.image(imagen_path, caption=ciudad, use_column_width=True)
        ...
        with col2:
            for idx, row in df_ciudad.iterrows():
                ...
                # Aplicar estilo para bordes negros y encabezados en negrita con fondo gris claro
                tabla_pred_style = tabla_pred.style.set_properties(**{
                    'border-color': 'black',
                    'border-width': '1px',
                    'border-style': 'solid'
                }).set_table_styles([
                    {'selector': 'th', 'props': [('background-color', '#f0f0f0'), ('color', 'black'), ('font-weight', 'bold'), ('border', '1px solid black')]}  # Encabezado en negrita y gris claro
                ])
                ...
                tabla_caracteristicas = pd.DataFrame({
                    "Precio": [int(row['price'])],  # Convertir a entero
                    "Tipo de Huésped": [row['type_host']],
                    "Número de Reseñas": [int(row['number_reviews'])],  # Convertir a entero
                    "Número de Huéspedes": [int(row['number_guest'])],  # Convertir a entero
                    "Número de Habitaciones": [int(row['number_bedroom'])],  # Convertir a entero
                    "Número de Camas": [int(row['number_beds'])],  # Convertir a entero
                    "Tipo de Baño": [row['type_bathroom']],
                    "Número de Baños": [int(row['number_bathroom'])]  # Convertir a entero
                })

                # Eliminar el índice del DataFrame
                tabla_caracteristicas = tabla_caracteristicas.reset_index(drop=True)

                # Aplicar el mismo estilo para la tabla de características adicionales
