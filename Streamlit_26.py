...
import os
from objetos.creacion_reseñas import extraer_datos_y_unir_2
...
def modelo_prediccion():
    ...
    st.markdown("<h1 style='text-align: center; color: darkblue;'>Modelo de Predicción de Precios de Airbnb</h1>", unsafe_allow_html=True)
    ...
    st.markdown("""# Estimación de Precios de Alojamientos en Airbnb 

Esta aplicación permite estimar el rango de precios de alojamientos en función de características específicas (como ciudad, tipo de baño, número de habitaciones y capacidad de huéspedes). Utiliza técnicas de **aprendizaje automático** para analizar los datos históricos de Airbnb y, a través de modelos de clasificación y regresión, determina patrones y relaciones en las características de los alojamientos.

Primero, asigna cada alojamiento a un clúster específico para reflejar mejor sus características y, luego, aplica un modelo de regresión optimizado para estimar el precio y su rango probable. Esto permite obtener una predicción más precisa y personalizada para cada alojamiento.
""")
    ...
    st.markdown("<p style='color: #7B7D7D;'>Completa los detalles de tu alojamiento para obtener una estimación de precio.</p>", unsafe_allow_html=True)
    ...
    if st.button("📊 Calcular Rango de Precios"):
        ...
        st.markdown(f"<h3 style='text-align: center; color: darkblue;'>Precio estimado: ${prediccion[0]:.2f}</h3>", unsafe_allow_html=True)
        st.markdown(f"<h4 style='text-align: center; color: #7B7D7D;'>Rango de precio estimado para su Airbnb: ${intervalo_inferior:.2f} - ${intervalo_superior:.2f}</h4>", unsafe_allow_html=True)
